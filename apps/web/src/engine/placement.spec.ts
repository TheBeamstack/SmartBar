/**
 * v1.0.6 N5 / Track U4 ([REF-UI-560]) — pure placement helpers (snap + fresh id + build). Headless,
 * no store/DOM (the store router `placeInSection` composes these; the UI is in `tools.spec.tsx`).
 */
import { describe, it, expect } from "vitest";
import { snapSection, freshPlacedId, buildPlacedBar, type SectionGeom } from "./placement";
import type { PlacedRowDoc, PlacedBundleDoc, PlacedLayerDoc } from "./document";

const GEOM: SectionGeom = { b: 400, h: 600, cover: 30, diameter: 20 };

describe("snapSection", () => {
  it("clamps a point outside the cover envelope back to the buildable limit", () => {
    // envelope half-limit u = 400/2 − 30 − 20/2 = 160; v = 600/2 − 30 − 10 = 260
    const s = snapSection(500, -1000, GEOM);
    expect(s.u).toBe(160);
    expect(s.v).toBe(-260);
  });

  it("snaps to the grid inside the envelope", () => {
    expect(snapSection(97, 12, GEOM)).toEqual({ u: 95, v: 10 }); // grid = 5
  });

  it("on a non-rect section (no b/h) only grid-snaps, no clamp", () => {
    const s = snapSection(9999, -9999, { cover: 25, diameter: 12 });
    expect(s).toEqual({ u: 10000, v: -10000 });
  });
});
...
describe("buildPlacedBar", () => {
  const base = { id: "p1", u: 40, v: -260, shapeId: "DROITE", diameter: 20, geom: GEOM };
